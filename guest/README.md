...
> 当前目录**不包含项目自建来宾镜像，也没有已完成的镜像构建脚本**。应用通过 `/api/probe-kernel` 使用 v86 官方列出的远程 Buildroot 演示探针；不得把它复制进仓库或冒充项目可重复构建的正式镜像。

当前探针为 10,068,480 bytes，记录的 SHA-256 是 `507a759c70ab7a490a233be454d0b5b88bc667956a410b531cb4edc091e2eb1c`。浏览器会用 Web Crypto 验证大小和哈希，再启动 BusyBox 来宾。该证据只说明字节完整且可运行，不代表本项目拥有可重复构建的正式来宾产物。
